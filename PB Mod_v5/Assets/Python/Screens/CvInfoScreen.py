...
#############################################################################################################
	...
	def buildScoreCache(self, scoreType):
		...
		if (p not in self.aiPlayersMet
				or not self.scoreDisplayFlag[p] ):
	...
	def drawLegend(self):
		...
		if self.scoreDisplayFlag[p]:
				...
				str = u"<color=%d,%d,%d,%d>%s</color>" %(128,128,128,255,name)
		...
		screen.setText( self.getNextWidgetName(), ""
			       ...
			       , 0, FontTypes.TITLE_FONT
			       , WidgetTypes.WIDGET_GENERAL, LEGENDLABEL_ID, p
			       )
...
################################## GRAPH TAB ###################################
		...
		# Something Clicked
			...
			if( inputClass.getData1() == LEGENDLABEL_ID ):
				playerId = inputClass.getData2()
				self.scoreDisplayFlag[playerId] = not self.scoreDisplayFlag[playerId]
				self.deleteAllLines()
				# Compute the scores
				self.reset()
				#self.buildScoreCache(self.iGraphTabID)
