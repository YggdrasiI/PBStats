...
const std::string getExeFilename(){
	...
	for( int i=0; i<n; ++i){
		if( GetFileAttributes( names[i].c_str() ) != INVALID_FILE_ATTRIBUTES ){
			return names[i];
		}
	}
...
int main(const int argc, const TCHAR * const argv[])
{
		...
		if( argc > 1 ){
			...
			if ( userExe.compare(userExe.length()-4, 4, ".exe") == 0
					&& GetFileAttributes( userExe.c_str() ) != INVALID_FILE_ATTRIBUTES ){
				argsStream << userExe;
				++iArg;
			}else{
