...
int Webserver_Default_Port = 8080;
	...
	if (len > 0x20) {
		...
		if (id == 0x0805 /* DirectIP game */ ||
			...
			if( pos_saves != std::string::npos ){
				...
				url_stream << "_http_:";
				url_stream <<  Webserver_Default_Port;
	...
	_Inout_opt_ int             *fromlen)
{
	...
	if (recv_from_length > 0x20) {
		...
		if (id == 0x0805 /* DirectIP game */ ||
			...
			if (str_filename[hostnameBegin + 2] == ':') {
...
extern "C" BOOL APIENTRY DllMain(HINSTANCE hinstDLL, DWORD dwReason, LPVOID lpvReserved)
{
	...
	case DLL_PROCESS_ATTACH:
	{
		...
		if( libvar != NULL ){
			Webserver_Default_Port = atoi(libvar);
		}

		startServer(Webserver_Default_Port);
#endif
