...
#
# Notes:
# - Step 2. and 3. require the PB Mod of this project.
...
class PBNetworkConnection:
    # Outgoing means server to client because this program runs
    # on the server :-)
...
def analyze_udp_traffic(device, ip_address, pcap_filter, connections, games, pcap_timeout):
    ...
    while True:
        ...
        if ip.source == ip_address:
            game = games.get(udp.sourceport)
            connections.get(ip.destination, udp.destinationport, ip.source, udp.sourceport, now).handle_server_to_client(payload, game, now)
        elif ip.destination == ip_address:
            game = games.get(udp.destinationport)
            connections.get(ip.source, udp.sourceport, ip.destination, udp.destinationport, now).handle_client_to_server(payload, game, now)
        else:
...
def main():
    ...
    while True:
        try:
            analyze_udp_traffic(args.network_interface, args.ip_address, portlist_to_filter(port_list),
