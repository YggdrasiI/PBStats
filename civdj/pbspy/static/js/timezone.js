...
// Use the above lib to create a cookie with the time zone of the
// user.
...
function getTimezoneList(e,current){
	...
	for ( var tz in jstz.olson.timezones ){
		...
		title = tzName;
		//Remove continent to short string
		//title = title.substring( title.indexOf("/")+1 );
		//limit displayed chars to prevent the shift of the scrollbar offscreen.
		if( title.length > 20 ){
...
/*
function getTimezoneListOld(e,current){
	var h = '<select size="1" onChange="setTimezone(this)" id="set_timezone_menu">';
	for ( var tz in jstz.olson.timezones ){
		tzName = jstz.olson.timezones[tz];
		title = tzName;
		//Remove continent to short string
...
function setTimezoneOld(e){
	tzName = e.options[e.selectedIndex].value;
	document.cookie = "timezone = " + tzName;
	e.outerHTML = '<span onClick="getTimezoneList(this,\''+tzName+'\');">'+tzName+'</span>';
}
*/
