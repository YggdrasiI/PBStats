...
PBMOD_ADVANCED_START_EXIT = 302017
...
class CvWorldBuilderScreen:
	...
	def handleInput (self, inputClass):				
		...
		elif (inputClass.getData1() == PBMOD_ADVANCED_START_EXIT):
			CyMessageControl().sendAdvancedStartAction(AdvancedStartActionTypes.ADVANCEDSTARTACTION_EXIT_CONFIRM,
					self.m_iCurrentPlayer, -1, -1, 1, False) #Action, Player, X, Y, Data, bAdd
			screen = CyGInterfaceScreen( "WorldBuilderScreen", CvScreenEnums.WORLDBUILDER_SCREEN )
			screen.hide("WorldBuilderExitButton2")
			screen.show("WorldBuilderExitButton")
...
#			if (not self.m_pCurrentPlot.isAdjacentNonrevealed(iActiveTeam) and self.m_pCurrentPlot.isRevealed(iActiveTeam, false)):
	...
	def clearSideMenu(self):
		...
		screen.deleteWidget("WorldBuilderExitButton2")
	...
	def setSideMenu(self):
		...
		if (CyInterface().isInAdvancedStart()):
			...
			szText = localText.getText("TXT_KEY_ADVANCED_START_BEGIN_GAME2", ())
			screen.setButtonGFC( "WorldBuilderExitButton2", szText, "", iX, iY,
					130, 28, WidgetTypes.WIDGET_GENERAL,
					PBMOD_ADVANCED_START_EXIT, -1, ButtonStyles.BUTTON_STYLE_STANDARD )
