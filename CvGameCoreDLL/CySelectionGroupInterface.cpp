...
void CySelectionGroupInterface()
{
	...
	python::class_<CySelectionGroup>("CySelectionGroup")
		...
		.def("pushMission", &CySelectionGroup::pushMission, "void (MissionTypes eMission, int iData1, int iData2, int iFlags, bool bAppend, bool bManual, MissionAITypes eMissionAI, CyPlot* pMissionAIPlot, CyUnit* pMissionAIUnit)")
		.def("pushMoveToMission", &CySelectionGroup::pushMoveToMission, "void (int iX, int iY)")
		.def("popMission", &CySelectionGroup::popMission, "void () - removes mission from queue")
		.def("lastMissionPlot", &CySelectionGroup::lastMissionPlot, python::return_value_policy<python::manage_new_object>(), "CyPlot* ()")
		...
		.def("canDoInterfaceMode", &CySelectionGroup::canDoInterfaceMode, "bool (InterfaceModeTypes eInterfaceMode)")
		.def("canDoInterfaceModeAt", &CySelectionGroup::canDoInterfaceModeAt, "bool (InterfaceModeTypes eInterfaceMode, CyPlot* pPlot)")

		.def("canDoCommand", &CySelectionGroup::canDoCommand, "bool (CommandTypes eCommand, int iData1, int iData2, bool bTestVisible) - can the group perform eCommand?")
		...
		.def("isInvisible", &CySelectionGroup::isInvisible, "bool (int (TeamTypes) eTeam)")
		.def("countNumUnitAIType", &CySelectionGroup::countNumUnitAIType, "int (UnitAITypes eUnitAI)")
		.def("hasWorker", &CySelectionGroup::hasWorker, "bool ()")

		.def("at", &CySelectionGroup::at, "bool (int iX, int iY) - is the group at plot iX, iY?")
		...
		.def("getNumUnits", &CySelectionGroup::getNumUnits, "int ()")
