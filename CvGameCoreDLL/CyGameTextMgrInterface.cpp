...
void CyGameTextMgrInterface()
{
	...
	python::class_<CyGameTextMgr>("CyGameTextMgr")
		...
		.def("getReligionHelpCity", &CyGameTextMgr::getReligionHelpCity, "wstring (int iReligion, CyCity* pCity, bool bCityScreen, bool bForceReligion, bool bForceState, bool bNoStateReligion)")
		...
		.def("getSpecialistHelp", &CyGameTextMgr::getSpecialistHelp, "wstring (int iSpecialist, bool bCivilopediaText)")
		.def("getTradeString", &CyGameTextMgr::getTradeString, "wstring (TradeData* pTradeData, int iPlayer1, int iPlayer2)")
